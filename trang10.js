...
// // Vẽ và cập nhật hiệu ứng sao nhấp nháy
// function drawStars() {
...
//         if (star.alpha <= 0 || star.alpha >= 1) star.delta *= -1; // đảo chiều nhấp nháy
//     }
...
// // Thay đổi kích thước canvas khi resize
...
function drawStars() {
    ...
    for (let star of stars) {
        ...
        ctx.fill();
        star.alpha += star.delta;
        if (star.alpha <= 0.5 || star.alpha >= 1.3) star.delta *= -1;
    }
    requestAnimationFrame(drawStars);
}
...
finishBtn.addEventListener("click", () => {
  window.location.href = "trang11.html";
});
